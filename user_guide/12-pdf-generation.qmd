...
nokap automatically detects when a table (or other element with intrinsic width) is being constrained by the viewport. It widens the viewport to let the element render at its natural size, then sizes the PDF to fit. No manual `vwidth` adjustment is needed — even very wide tables with many columns render completely.
